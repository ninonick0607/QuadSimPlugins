// Copyright 2021-2024 Rapyuta Robotics Co., Ltd.
// This code has been autogenerated from std_msgs/msg/Header.msg - do not modify

#pragma once

// UE
#include "CoreMinimal.h"

// ROS
#include "std_msgs/msg/header.h"

// rclUE
#include "Msgs/ROS2GenericMsg.h"
#include "rclcUtilities.h"

// Generated Msg/Srv/Action(can be empty)
#include "Msgs/ROS2Time.h"

// Generated
#include "ROS2StdHeader.generated.h"

USTRUCT(Blueprintable)
struct RCLUE_API FROSStdHeader
{
    GENERATED_BODY()

public:
    UPROPERTY(EditAnywhere, BlueprintReadWrite)
    FROSTime Stamp;

    UPROPERTY(EditAnywhere, BlueprintReadWrite)
    FString FrameId;

    FROSStdHeader()
    {
    }

    void SetFromROS2(const std_msgs__msg__Header& in_ros_data)
    {
        Stamp.SetFromROS2(in_ros_data.stamp);

        FrameId = UROS2Utils::StringROSToUE<rosidl_runtime_c__String>(in_ros_data.frame_id);
    }

    void SetROS2(std_msgs__msg__Header& out_ros_data) const
    {
        Stamp.SetROS2(out_ros_data.stamp);

        UROS2Utils::StringUEToROS(FrameId, out_ros_data.frame_id);
    }
};

UCLASS()
class RCLUE_API UROS2StdHeaderMsg : public UROS2GenericMsg
{
    GENERATED_BODY()

public:
    virtual void Init() override;
    virtual void Fini() override;

    virtual const rosidl_message_type_support_t* GetTypeSupport() const override;

    UFUNCTION(BlueprintCallable)
    void SetMsg(const FROSStdHeader& Input);

    UFUNCTION(BlueprintCallable)
    void GetMsg(FROSStdHeader& Output) const;

    virtual void* Get() override;

private:
    virtual FString MsgToString() const override;

    std_msgs__msg__Header header_msg;
};
